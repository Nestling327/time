            ...
            with open(time_file_path, 'r') as f:
                ...
                for line in f:
                    ...
                    if len(delta) != len(sentences[count]):
                        print 'Data conflict at line {}, delete'.format(count)
                        logging.info('Data conflict at line {}, delete'.format(count))
                        sentences[count] = sentences[count][0:len(delta)]
...
def prepare_data(data, vocab_size, one_hot=False, sigmoid_on=False):
    ...
    if type(y_origin[0]) is list:
        # train
        y = np.zeros((n_samples, max_len)).astype('int8')
    ...
    if one_hot:
        print 'n_samples:{} max_len:{} vocab_size:{}'.format(n_samples, max_len, vocab_size)
        logging.info('n_samples:{} max_len:{} vocab_size:{}'.format(n_samples, max_len, vocab_size))
