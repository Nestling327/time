...
parser.add_argument('--vocab_size', type=int, default=2500, help='Vocabulary size')
parser.add_argument('--max_len', type=int, default=2000, help='Maximum length of the sequence.')
...
if DEBUG:
    logging.basicConfig(filename='log/DEBUG-{}-{}-{}.log'.format(MODEL_TYPE, DATA_TYPE,
    ...
    logging.basicConfig(
        level=logging.INFO, format=FORMAT,
        filename='log/{}-{}-{}-{}.log'.format(MODEL_TYPE, DATA_TYPE, N_HIDDEN,
...
def main(num_epochs=NUM_EPOCHS, vocab_size=VOCAB_SIZE):
    ...
    def evaluate(model, current_epoch, additional_test_length):
        ...
        if not DEBUG:
            utils.save_model('{}-{}-{}-{}'.format(MODEL_TYPE, current_epoch, DATA_TYPE, N_HIDDEN),
        ...
        while True:
            ...
            if it % PRINT_FREQ == 0:
                logging.info("Epoch {}-{},iter {} average seq length = {} average loss = {}".format(current_epoch, (
                        it - last_it) * 1.0 * BATCH_SIZE / data_size,
                                                                                                    it,
                                                                                                    avg_seq_len / PRINT_FREQ,
                                                                                                    avg_cost / PRINT_FREQ))
                avg_cost = 0
                avg_seq_len = 0
        logging.info('End')
